...
export class SearchUser extends React.Component {
  ...
  setRecentSearches(obj) {
    ...
    recentSearches = recentSearches.filter(search => search.username !== obj.username);
  ...
  getFilteredList = (prefix) => this.state.userList.filter(user => user.name.startsWith(prefix))
      ...
      case 38: // up arrow
        this.setState({
          selectedKey: selectedKey > 0 ? (--selectedKey) :
            searchContentLength + Math.max(--selectedKey, -1) % searchContentLength,
        });
        break;
      case 40: // down arrow
        this.setState({
          selectedKey: selectedKey >= -1 ? (++selectedKey) % searchContentLength :
            searchContentLength + (++selectedKey),
        });
        break;
      case 13: // enter key
        if (selectedKey > -1) {
          (selectedKey < filteredList.length) && this.setRecentSearches(filteredList[selectedKey]);
          this.setState({
            name: selectedKey < filteredList.length ? filteredList[selectedKey].name : this.recentSearches[selectedKey - filteredList.length].name,
            username: selectedKey < filteredList.length ? filteredList[selectedKey].username : this.recentSearches[selectedKey - filteredList.length].username,
            filteredList: [],
            selectedKey: -1,
            expand: false
          })
          ...
          <div className="user-suggestion-box">
            {filteredList.map(
              ...
              <div
                ...
                className={`user-suggestion ${selectedKey === i ? 'user-suggestion-active' : ''}`}
                key={i}
              >
                {suggestion.name}
              </div>
            ))}
            <label className="user-suggestion-recent-searches">Recent searches</label>
            {this.recentSearches.map(
              (suggestion, i) => (
                <div
                  ...
                  className={`user-suggestion ${selectedKey === (i + length) ? 'user-suggestion-active' : ''}`}
                  key={i}
                >
                  {suggestion.name}
                </div>)
